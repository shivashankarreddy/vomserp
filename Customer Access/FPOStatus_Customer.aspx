<%@ Page Title="" Language="C#" MasterPageFile="~/CustomerMaster.master" AutoEventWireup="true"
    CodeBehind="FPOStatus_Customer.aspx.cs" Inherits="VOMS_ERP.Customer_Access.FPOStatus_Customer" %>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            ...
            oTable = $("[id$=gvItmMstr]").dataTable({
                ...
                "bServerSide": true,
                "bDestroy": true,
                "sAjaxSource": "PI_Webservice.asmx/GetFPOItems",
                "bJQueryUI": true,
        ...
        function Reapply(valddd, CreatedBy, IsCust, IsVerbalLPO, Status) {
            try {
                var Result = '';
                if (Status != "100") {
                    if (confirm("Are you sure to Cancel the FPO No : " + valddd.parentNode.parentNode.childNodes[0].innerText + "...?")) {
                        if (Status <= "50") {
                            Result = FPOStatus_Customer.Cancel(valddd.parentNode.parentNode.id, CreatedBy, IsCust, IsVerbalLPO);
                            oTable.fnDraw();
        ...
        function Ammendement(valddd, CreatedBy, IsCust, IsVerbal) {
            try {
                var result = FPOStatus_Customer.EditItemDetails(valddd.parentNode.parentNode.id, CreatedBy, IsCust);
                var fres = result.value;
                if (fres == 'Success') {
                    if (IsVerbal == 0)
                        window.location.replace("../Customer Access/NewFPO_Customer.aspx?ID=" + valddd.parentNode.parentNode.id + "&IsAm=True");
                    else
                        window.location.replace("../Customer Access/NewFPO_Customer.aspx?ID=" + valddd.parentNode.parentNode.id + "&IsAm=True");
        ...
        function EditDetails(valddd, CreatedBy, IsCust, IsVerbal) {
            try {
                ...
                if (fres == 'Success') {
                    if (IsVerbal == 0)
                        window.location.replace("../Customer Access/NewFPO_Customer.aspx?ID=" + valddd.parentNode.parentNode.id);
                    else
        ...
        function Delet(valddd, CreatedBy, IsCust) {
            try {
                if (confirm("Are you sure you want to Delete?")) {
                    var result = FPOStatus_Customer.DeleteItemDetails(valddd.parentNode.parentNode.id, CreatedBy, IsCust);
                    var fres = result.value;
