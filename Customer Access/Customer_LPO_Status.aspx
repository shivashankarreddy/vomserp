<%@ Page Title="" Language="C#" MasterPageFile="~/CustomerMaster.master" AutoEventWireup="true"
    CodeBehind="Customer_LPO_Status.aspx.cs" Inherits="VOMS_ERP.Customer_Access.Customer_LPO_Status" %>
    ...
    <script type="text/javascript">
        ...
        function DrawTable() {
            ...
            oTable = $("[id$=gvLpoStatus]").dataTable({
                ...
                "bServerSide": true,
                "bDestroy": true,
                "sAjaxSource": "Customer_LPO_WebService.asmx/GetCustomerLPOItems",
        ...
        function mailsDetails(valddd) {
            try {
                ...
                window.location.replace("../Masters/EmailSend.aspx?LpoID_Customer=" + valddd.parentNode.parentNode.id);
            } catch (e) {
                alert(e.Message);
            }
        }
        ...
        function Reapply(valddd, CreatedBy, IsCust, IsVerbalLPO, Status) {
            try {
                var Result = '';
                if (Status != "101") {
                    if (confirm("Are you sure to Cancel the LPO No : " + valddd.parentNode.parentNode.childNodes[0].innerText + "...?")) {
                        if (Status <= "60") {
                            Result = Customer_LPO_Status.Cancel(valddd.parentNode.parentNode.id, CreatedBy, IsCust, IsVerbalLPO);
                            oTable.fnDraw();
        ...
        function EditDetails(valddd, CreatedBy, IsCust, IsVerbal, Status) {
            try {
                var result = Customer_LPO_Status.EditItemDetails(valddd.parentNode.parentNode.id, CreatedBy, IsCust);
                var fres = result.value;
                if (fres == 'Success') {
                    if (IsVerbal == false) {
                        if (Status >= 70) {
                            ErrorMessage("Unable to Edit GRN Processed for this reocrd / LPO is Cancelled");
                        }
                        else
                            window.location.replace("../Customer Access/Customer_LPO.Aspx?ID=" + valddd.parentNode.parentNode.id);
                    }
                    else if (IsVerbal == true)
                        window.location.replace("../Customer Access/VerbalLPO_Customer.Aspx?ID=" + valddd.parentNode.parentNode.id);
                    else
        ...
        function Ammendement(valddd, CreatedBy, IsCust, IsVerbal) {
            try {
                ...
                if (fres == 'Success') {
                    if (IsVerbal == false)
                        window.location.replace("../Customer Access/Customer_LPO.Aspx?ID=" + valddd.parentNode.parentNode.id + "&IsAm=True");
                    else {
        ...
        function Delet(valddd, CreatedBy, IsCust) {
            try {
                if (confirm("Are you sure you want to Delete?")) {
                    var result = Customer_LPO_Status.DeleteItemDetails(valddd.parentNode.parentNode.id, CreatedBy, IsCust);
                    var fres = result.value;
