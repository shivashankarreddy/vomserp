    ...
    public class InvoicesWebService1 : System.Web.Services.WebService
    {
        ...
        public string GetPInvStatus()
        {
            try
            {
                ...
                if (SInvNo != "")
                    s.Append(" and iom.ShpmntPrfmaInvc LIKE '%" + SInvNo.Replace("'", "''") + "%'");
                if (PInvNo != "")
                    s.Append(" and p.PrfmInvcNo LIKE '%" + PInvNo.Replace("'", "''") + "%'");
                if (Trms != "")
                    s.Append(" and TrmsDlvryPmnt LIKE '%" + Trms.Replace("'", "''") + "%'");
                if (CustNm != "")
                    s.Append(" and  dbo.GetActiveCustomerName(p.CustomerID) LIKE '%" + CustNm.Replace("'", "''") + "%'");
                if (RefFPO != "")
                    s.Append(" and dbo.FN_MergeTableColumnFPO(p.FPOs) LIKE '%" + RefFPO.Replace("'", "''") + "%'");
                if (ShpPlngNo != "")
                    s.Append(" and cl.ChkLstRefNo LIKE '%" + ShpPlngNo.Replace("'", "''") + "%'");
                if (Stat != "")
                    s.Append(" and dbo.FN_GetStatus(p.StatusID) LIKE '%" + Stat.Replace("'", "''") + "%'");
                ...
                string query = @"  
                            ...
                            INTO
	                            @MAA (ID,PrfmInvcNo,PrfmaInvcDt,TrmsDlvryPmnt,CustomerName,
	                              ...
	                              SELECT p.ID, p.PrfmInvcNo, PrfmaInvcDt   
                                    as PrfmaInvcDt, TrmsDlvryPmnt, dbo.GetActiveCustomerName(p.CustomerID) as CustomerName,   
                                    dbo.FN_MergeTableColumnFPO(p.FPOs) as FPONmbrs,   
                                    iom.ShpmntPrfmaInvc as InvoiceNo, dbo.FN_GetStatus(p.StatusID) as Status,cl.ChkLstRefNo as CheckListNo,
                                    '<img src=../images/Edit.jpeg alt=Edit onclick=EditDetails(this,''' + CONVERT(varchar(40), p.CreatedBy) 
                                     +''','+(case when u.ContactType = 'Customer' then '1' else '0' end) +') />' EDIT, 
                                     '<img src=../images/delete.png alt=Delete onclick=Delet(this,'''
                                     + CONVERT(varchar(40), p.CreatedBy) +''','+(case when u.ContactType = 'Customer' then '1' else '0' end) +','''+Convert(nvarchar(max),p.CompanyId) +''') />' Delt, p.CompanyId ,p.CreatedDate
                                    FROM ProformaInvoice p  
                                    inner join CheckList cl on p.CheckLIstID = cl.ChkListID
                                    inner join Users u on u.ID = p.CreatedBy        
                                    left join IOMTemplate2 iom on p.ID = iom.PrfmaInvcID
