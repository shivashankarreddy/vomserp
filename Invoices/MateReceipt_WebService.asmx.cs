    ...
    public class MateReceipt_WebService : System.Web.Services.WebService
    {
        ...
        public string GetMateReceiptStatus()
        {
            try
            {
                ...
                if (PInvNo != "")
                    s.Append(" and PInvcNmbr LIKE '%" + PInvNo + "%'");
                if (SBNo != "")
                    s.Append(" and ShpngBilNumber LIKE '%" + SBNo + "%'");
                if (date != null && date != "")
                {
                ...
                string query = @"  
                            ...
                            INTO
	                            @MAA ( ID, PInvNO, SBNo, Date, ReceiptNo, ForwarderName, EDIT, Delt, CreatedDate )
	                               Select ID, PInvcNmbr, ShpngBilNumber, MRDate, MReceiptNmbr, ForwarderName, EDIT, Delt, CreatedDate from View_MateReceipt MR  where MR.IsActive <>0
                                   {4}                   
