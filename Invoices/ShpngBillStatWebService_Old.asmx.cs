    ...
    public class ShpngBillStatWebService : System.Web.Services.WebService
    {
        ...
        public string GetShippingBillStat()
        {
            try
            {
                ...
                if (PrtLoading != "")
                    s.Append(" and dbo.FN_GetDescription(SBD.PrtLoading) LIKE '%" + PrtLoading + "%'");
                if (PrtDischarge != "")
                    s.Append(" and dbo.FN_GetDescription(SBD.PrtDischarge) LIKE '%" + PrtDischarge + "%'");
                if (CntryDestination != "")
                    s.Append(" and dbo.FN_GetDescription(SBD.CntryDestination) LIKE '%" + CntryDestination + "%'");
                if (CntryOrigine != "")
                    s.Append(" and dbo.FN_GetDescription(SBD.CntryOrigine) LIKE '%" + CntryOrigine + "%'");
                ...
                string query = @"  
                            ...
                            INTO
	                            @MAA ( ID ,FS_AdrsDtlsID ,DBK_DtlsID ,INVC_DtlsID ,DEPB_DtlsID ,DA_DtlsID ,RefInvcID ,
	                                ...
	                                SELECT SBD.ID, SBD.FS_AdrsDtlsID, SBD.DBK_DtlsID, SBD.INVC_DtlsID, SBD.DEPB_DtlsID,SBD.DA_DtlsID, SBD.RefInvcID,
                                    SBD.ShpngBilNmbr,SBD.ShpngBilDate,INVD.PrfmaInvoiceNmbr,
                                    INVD.PrfmaInvoiceDate, dbo.FN_GetDescription(SBD.PrtLoading) as PrtLoadingDes, 
                                    dbo.FN_GetDescription(SBD.PrtDischarge) as PrtDischargeDes,
                                    dbo.FN_GetDescription(SBD.CntryDestination) as CntryDestinationDes, 
                                    dbo.FN_GetDescription(SBD.CntryOrigine) as CntryOrigineDes, sbd.CreatedBy,
                                    '<img src=../images/Edit.jpeg alt=Edit onclick=EditDetails(this,'''
                                    + CONVERT(nvarchar(max), SBD.CreatedBy) +''','+(case when u.ContactType = 'Trafficker' then '1' else '0' end) +') />' EDIT, 
                                    '<img src=../images/delete.png alt=Delete onclick=Delet(this,'''
                                    + CONVERT(nvarchar(max), SBD.CreatedBy) +''','+(case when u.ContactType = 'Trafficker' then '1' else '0' end) +') />' Delt, SBD.CreatedDate
                                    FROM ShippingBillDetails SBD 
                                    INNER JOIN FS_AddresDetails FSA ON SBD.FS_AdrsDtlsID = FSA.ID 
                                    INNER JOIN DBK_Details DBKD ON SBD.DBK_DtlsID = DBKD.ID AND FSA.DBK_DtlsID = DBKD.ID 
                                    INNER JOIN DEPB_Details DEPB ON SBD.DEPB_DtlsID = DEPB.ID 
	                                    AND FSA.DEPB_DtlsID = DEPB.ID AND DBKD.DEPB_DtlsID = DEPB.ID 
                                    INNER JOIN INVC_Details INVD ON SBD.INVC_DtlsID = INVD.ID 
	                                    AND FSA.INVC_DtlsID = INVD.ID AND DBKD.INVC_DtlsID = INVD.ID
                                    INNER JOIN DA_Details DAD ON SBD.DA_DtlsID = DAD.ID AND FSA.DA_DtlsID = DAD.ID 
	                                    AND INVD.DA_DtlsID = DAD.ID 
                                    INNER JOIN Users U ON U.ID = SBD.CreatedBy 
	                                {4}                   
