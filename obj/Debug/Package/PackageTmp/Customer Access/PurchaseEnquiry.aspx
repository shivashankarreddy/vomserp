<%@ Page Title="" Language="C#" MasterPageFile="~/CustomerMaster.master" AutoEventWireup="true"
    CodeBehind="PurchaseEnquiry.aspx.cs" Inherits="VOMS_ERP.Customer_Access.PurchaseEnquiry" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
                    ...
                    <tr class="bcTRTitleRow">
                        <td class="bcTdTitleLeft" align="left" colspan="6">
                            <table width="100%">
                                <tr>
                                    <td>
                                        &nbsp;&nbsp;&nbsp;<asp:Label ID="lblTitle" runat="server" Text="Purchase Enquiry"
                        ...
                        <td>
                            <table width="100%" style="background-color: #F5F4F4; padding: 5px; border: solid 1px #ccc">
                                ...
                                <tr>
                                    ...
                                    <td class="bcTdnormal">
                                        <asp:DropDownList runat="server" ID="ddlsubject" ValidationGroup="D" CssClass="bcAspdropdown">
                                ...
                                <tr>
                                    <td class="bcTdnormal">
                                        <span id="Span5" class="bcLabelright">Purchase Indent Number<font color="red" size="2"><b>*</b></font>:</span>
                                    </td>
                                    <td class="bcTdnormal">
                                        <asp:TextBox runat="server" ID="txtPINumber" ValidationGroup="D" CssClass="bcAsptextbox"
                                            onkeypress="return isSomeSplChar(event)" MaxLength="350" onchange="javascript:return CheckEnquiryNo();"></asp:TextBox>
                                    </td>
                                    <td class="bcTdnormal">
                                        <span id="Span6" class="bcLabelright">Purchase Indent Date<font color="red" size="2"><b>*</b></font>:</span>
                                    </td>
                                    <td class="bcTdnormal">
                                        <asp:TextBox runat="server" ID="txtPIDate" ValidationGroup="D" CssClass="bcAsptextbox"
                                            onchange="changedateEnqDT();"></asp:TextBox>
                                    </td>
                                </tr>
                                <tr>
                                    <td class="bcTdnormal">
                                        <span id="lblEnquiry" class="bcLabelright">Purchase Enquiry Number<font color="red"
                                            size="2"><b>*</b></font>:</span>
                                    </td>
                                    <td class="bcTdnormal">
                                        <asp:TextBox runat="server" ID="txtenqno" Enabled="false" ValidationGroup="D" CssClass="bcAsptextbox"
                                            onkeypress="return isSomeSplChar(event)" MaxLength="350" onchange="javascript:return CheckEnquiryNo();"></asp:TextBox>
                                    </td>
                                    <td class="bcTdnormal">
                                        <span id="lblEnquirydt" class="bcLabelright">Purchase Enquiry Date<font color="red"
                                            size="2"><b>*</b></font>:</span>
                                    </td>
                                    <td class="bcTdnormal">
                                        <asp:TextBox runat="server" ID="txtenqdt" ValidationGroup="D" CssClass="bcAsptextbox"></asp:TextBox>
                                    </td>
    ...
    <script type="text/javascript">
        ...
        function changedateEnqDT() {
            var strdateEnqDT = $('[id$=txtPIDate]').val();
        ...
        function Myvalidations() {
            try {
                ...
                else if (($('[id$=ddlsubject]').val()).trim() == '0') {
                    ErrorMessage('Subject is Required.');
                    $('[id$=ddlsubject]').focus();
                    return false;
                }
                else if (($('[id$=txtPINumber]').val()).trim() == '') {
                    ErrorMessage('Purchase Indent Number is Required.');
                    $('[id$=txtPINumber]').focus();
                    return false;
                }
                else if (($('[id$=txtPIDate]').val()).trim() == '') {
                    ErrorMessage('Purchase Indent Date is Required.');
                    $('[id$=txtPIDate]').focus();
                    return false;
                }
                else if (($('[id$=txtenqdt]').val()).trim() == '') {
                    ErrorMessage('Purchase Enquiry Date is Required.');
                    $('[id$=txtenqdt]').focus();
                    return false;
                }
        ...
        function UpdateSelectedItem() {
            ...
            if (ItemID != '00000000-0000-0000-0000-000000000000' && (Qty != 0 || Qty != '') && UnitID != 0) {
                ...
                var result = PurchaseEnquiry.UpdateSelectedItem(selVal, ItemID, PartNo, Spec, Make, Qty, UnitID);
        ...
        $('[src*=expand]').live('click', function (e) {
            ...
            var NewTbl = PurchaseEnquiry.Expand_FEs(ItemID, TRid);
        ...
        function GetItemDesc_Spec(ItmID, id) {
            var rslt = PurchaseEnquiry.GetItemDesc_Spec(ItmID, id);
        ...
        function Add_Sub_Itms1(TrID, RowID, ClickRID, RowIndex, IsAdd) {
            ...
            if (IsAdd == true) {
                if (ItemID.trim() == "" || ItemID.trim() == 0) {
                    ...
                    var res;
                    res = PurchaseEnquiry.Add_Sub_Itms(RowID, TrID, SubRowID, ParentItemID, ItemID, ItmDesc, Spec, PNo, Make, Qty, UnitID, IsAdd);
        ...
        function Delete_SubItem(TrID, SNo, x) {
            if (confirm("Are you sure you want to Delete?")) {
                ...
                var ParentItemID = $('#hfItemID' + TrID).val();
                var ItemID = $('#hfSubItemID' + ClickRID).val();
                var res = PurchaseEnquiry.Delete_SubItem(ParentItemID, TrID, SNo, ItemID);
                $(".DEL" + TrID).remove();
        ...
        function uploadComplete() {
            var result = PurchaseEnquiry.AddItemListBox();
        ...
        function AddItemRow(RowID) {
         var ITIMID = '00000000-0000-0000-0000-000000000000';
            ...
            if ((ItemID != 0 || ItemID != "") && (parseFloat(Qty) != 0 && Qty != '') && UnitID != 0) {
                //int RowID, int ItemID, string PartNo, string Spec, string Make, string Qty, int UnitID
                var result = PurchaseEnquiry.FillItemGrid(RowID, ItemID, PartNo, Spec, Make, Qty, UnitID, IsSubitem);
                var getDivFEItems = GetClientID("divFEItems").attr("id");
                $('#tblItems tbody').html('');
                ...
                if (ItemID == '00000000-0000-0000-0000-000000000000' || ItemID == "") {
                    ErrorMessage('ItemDescription is Required.');
                    $('#ddl' + RowID).focus();
                }
                else if (Qty == '') {
        ...
        var gett = "";
        function GetValueFromChilds(myVal) {
            gett = myVal;
            gett = gett.split(',');
            $('#ddl' + id).val(gett[0].toLowerCase())
        ...
        function fnOpen(id) {
            ...
            returnVal = window.open("../Customer Access/Additem_PI.aspx", "Add Item",
...
//                    FillSpec_ItemDesc(id);
                    ...
                    var result = PurchaseEnquiry.NewItemAdded();
                    var getDivFEItems = GetClientID("divFEItems").attr("id");
                    $('#' + getDivFEItems).html(result.value);
//                    $('#ddl' + id).val(rtnVal[0]);
//                    FillSpec_ItemDesc(id);
                ...
                returnVal = "";
                  var result = PurchaseEnquiry.FillSpec_ItemDesc(rtnVal[0]);
            var arr = result.value.split('&@&');
            $('#Txt_IM').val(arr[1]);
            $('#txtPartNo0').val(arr[2]);
            $('#txtSpec0').val(arr[3]);
            $('[id$=HFIsSubItem]').val(arr[4]);
            //$('#ddlUnits' + ID).val('103');
            SelectDDLunits(0);
...
//                $('#Txt_IM').val(arr[1]);
        ...
        function FillItemGrid(obj1, obj2) {
            ...
            if (obj3 == '' && flag == '') {
                ...
                if ((qty != '' && qty != '.')) {//|| obj4 != '0'                
                    var result = PurchaseEnquiry.FillItemGrid(obj2, obj1, obj3, 0, 274, obj5, spec, make, qty, PNo, returnVal);
                    var getDivFEItems = GetClientID("divFEItems").attr("id");
                    $('#' + getDivFEItems).html(result.value);
        ...
        function DeleteItem(obj1) {
            var result = PurchaseEnquiry.DeleteItem(obj1);
            var getDivFEItems = GetClientID("divFEItems").attr("id");
            $('#' + getDivFEItems).html(result.value);
            var listid = GetClientID("lbItems").attr("id");
            $('#' + listid)[0].selectedIndex = '0';
        ...
        function AddItemColumn(obj1, obj2) {
            ...
            if (obj3 != '') {
                if (obj3 != '' && obj3 > 0) {
                    var result = PurchaseEnquiry.AddItemColumn(obj2 + 1, obj1, obj3);
                    var getDivFEItems = GetClientID("divFEItems").attr("id");
                    $('#' + getDivFEItems).html(result.value);
                }
        ...
        function EditSelectedItem(RowID) {
            ...
            var result = PurchaseEnquiry.EditItemRow(SelID, ItemID);
            var arr = result.value.split('&@&');
...
//            $('#ddl' + RowID).val(arr[1]);
        ...
        function doConfirm(id) {
            ...
            var Cst = PurchaseEnquiry.CheckStat(ItmId, id);
            if (Cst.value == true) {
                if (confirm("Are you sure you want to Continue?")) {
                    var result = PurchaseEnquiry.DeleteItem(ItmId);
                    var getDivFEItems = GetClientID("divFEItems").attr("id");
                    $('#' + getDivFEItems).html(result.value);
                }
                else {
        ...
        $('#lnkdelete').click(function () {
            if ($('#lbItems').val() != null) {
                ...
                if (IsDelete) {
                    var result = PurchaseEnquiry.DeleteItemListBox($('#lbItems').val());
        ...
        function CheckEnquiryNo() {
            var enqNo = $('[id$=txtPINumber]').val();
            var result = PurchaseEnquiry.CheckEnquiryNo(enqNo);
            if (result.value == false) {
                $("#<%=txtPINumber.ClientID%>")[0].value = '';
                ErrorMessage('Enquiry Number Exists.');
                $("#<%=txtPINumber.ClientID%>")[0].focus();
                return false;
            }
            else
                return true;
            ...
            $('#Txt_IM').autocomplete({
                ...
                source: function (request, response) {
                    $.ajax({
                        ...
                        url: "PurchaseEnquiry.aspx/GetAutoCompleteData",
                ...
                select: function (event, ui) {
                    var ID = ui.item.val;
                   var result = PurchaseEnquiry.FillSpec_ItemDesc(ID);
            var arr = result.value.split('&@&');            
            $('#Txt_IM').val(arr[1]);
            $('#txtPartNo0').val(arr[2]);
            $('#txtSpec0').val(arr[3]);
