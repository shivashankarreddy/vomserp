<%@ Page Title="" Language="C#" MasterPageFile="~/CustomerMaster.master" AutoEventWireup="true"
    CodeBehind="NewFPOrderVendor.aspx.cs" Inherits="VOMS_ERP.Purchases.NewFPOrderVendor" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
                        ...
                        <td>
                            <table width="100%" style="background-color: #F5F4F4; padding: 15px; border: solid 1px #ccc"
                                ...
                                <tr>
                                    ...
                                    <td class="bcTdnormal">
                                        <span class="bcLabel">Purchase Enquiry Number<font color="red" size="2"><b>*</b></font>:</span>
                                    </td>
                                    <td class="bcTdnormal">
                                    ...
                                    <td class="bcTdnormal">
                                        <span class="bcLabel">Purchase Enquiry Date<font color="red" size="2"><b>*</b></font>:</span>
                                    </td>
                                    <td>
                                ...
                                <tr>
                                    ...
                                    <td class="bcTdnormal">
                                        <asp:DropDownList runat="server" ID="ddlVendor" CssClass="bcAspdropdown" OnSelectedIndexChanged="ddlVendor_SelectedIndexChanged"
                                            AutoPostBack="true">
                                            <asp:ListItem Value="00000000-0000-0000-0000-000000000000" Text="Select Vendor"></asp:ListItem>
                                        </asp:DropDownList>
                                    </td>
                                    <td class="bcTdnormal">
                                        <span class="bcLabel">FQuotation No<font color="red" size="2"><b>*</b></font>:</span>
                                    </td>
                                    <td>
                                        <asp:ListBox runat="server" ID="Lstfqn" SelectionMode="Multiple" CssClass="bcAspMultiSelectListBox"
                                            AutoPostBack="true" OnSelectedIndexChanged="Lstfqn_SelectedIndexChanged"></asp:ListBox>
                                    </td>
                                </tr>
                                <tr>
                                    <td class="bcTdnormal">
                                        ...
                                        <table style="width: 100%; overflow: auto;">
                                            ...
                                            <tr>
                                                <td colspan="8" class="bcTdNewTable">
                                                    <center>
                                                        <div style="overflow: auto; width: 35%; max-height:250px;" id="divPaymentTerms" runat="server">
                                                        </div>
    ...
    <script type="text/javascript">
        ...
        function uploadComplete() {
            var result = NewFPOrderVendor.AddItemListBox();
        ...
        $('#lnkdelete').click(function () {
            if ($('#lbItems').val() != null) {
                ...
                if (IsDelete) {
                    var result = NewFPOrderVendor.DeleteItemListBox($('#lbItems').val());
    ...
    <script type="text/javascript">
        ...
        function doConfirmPayment(id) {
            if (confirm("Are you sure you want to Delete Payment?")) {
                var result = NewFPOrderVendor.PaymentDeleteItem(id);
        ...
        function getPaymentValues(RNo) {
            ...
            if (PercAmt != '0' && PercAmt != '' && Desc != '') {
                var result = NewFPOrderVendor.PaymentAddItem(RNo, PercAmt, Desc);
                var getdivPaymentTerms = GetClientID("divPaymentTerms").attr("id");
                $('#' + getdivPaymentTerms).html(result.value);
                if ($('[id$=HfMessage]').val() != '') {
    ...
    <script type="text/javascript">
        ...
        function doConfirm(id) {
            if (confirm("Are you sure you want to Delete?")) {
                var result = NewFPOrderVendor.DeleteItem(id);
        ...
        function doConfirmRegret(id) {
            if (confirm("Are you sure you want to Regret?")) {
                ...
                var result = NewFPOrderVendor.RegretItem(id, fenqid);
                if (result.value == true) {

                    $('[id$=divMyMessage]').append('<span class="Success">Item Regretted Successfully.');


                    var result1 = NewFPOrderVendor.BindGridView(0);
                    var getDivFEItems = GetClientID("divFPOItems").attr("id");
        ...
        function CheckQTY(id, isnew) {
            var txtQuantity = $('#txtQuantity' + id);
            var txtQTY = $('#hfQTY' + id);
            if (parseFloat(txtQuantity.val()) > parseFloat(txtQTY.val())) {
                if (!confirm("Are you sure you want to enter more than '" + txtQTY.val() + "' Quantity...?")) {
                    txtQuantity.val(0);
                    txtQTY.val(0);
                }
            }
            AddItemColumn(id, isnew);
        }
        ...
        function AddItemColumn(id, isnew) {
            ...
            //btnSave
            if (sv == undefined && PrtNo == '' && spec == '' && Make == '' && Qty == '0' && Rate == 0) {
                ...
                if (Rate == undefined || Rate == '0')
                    Rate = 0;
                //var test = e.tagName; 
                var result = NewFPOrderVendor.AddNewRow(id, sv, PrtNo, spec, Make, Qty, Rate, Units, Rmrks, isnew);
                var getDivFEItems = GetClientID("divFPOItems").attr("id");
                $('#' + getDivFEItems).html(result.value);
            }
            Expnder();
        ...
        function NextPage() {
            ...
            if (ddlRowsChanged == 'NAN' || ddlRowsChanged <= 0) {
                ...
                var getDivFEItems = GetClientID("divFPOItems").attr("id");
                var result = NewFPOrderVendor.NextPage(hfCurrentPage, ddlRowsChanged);
                $('#' + getDivFEItems).html(result.value);
                Expnder();
            }
        };
        ...
        function PrevPage() {
            ...
            if (ddlRowsChanged == 'NAN' || ddlRowsChanged <= 0) {
                ...
                var result = NewFPOrderVendor.PrevPage(hfCurrentPage, ddlRowsChanged);
                var getDivFEItems = GetClientID("divFPOItems").attr("id");
                $('#' + getDivFEItems).html(result.value);
                Expnder();
            }
        ...
        function RowsChanged() {
            ...
            var result = NewFPOrderVendor.RowsChanged(hfCurrentPage, ddlRowsChanged);
            var getDivFEItems = GetClientID("divFPOItems").attr("id");
            $('#' + getDivFEItems).html(result.value);
            Expnder();
        }
        ...
        function DisplayRefFPO() {
            if ($('[id$=chkbIRO]')[0].checked) {
                ...
                window.location = "NewFPOrder.aspx";
            }
            var result = NewFPOrderVendor.ChkBoxRptdFPOMode($('[id$=chkbIRO]')[0].checked);
        }
        function FillItemDRP(id) {
            var ddlCat = GetClientID("ddl" + (parseInt(id))).attr("id");
            var sv = $('#' + ddlCat).val();
            var result = NewFPOrderVendor.FillItemDRP(id, sv);
            var getDivFEItems = GetClientID("divFPOItems").attr("id");
            $('#' + getDivFEItems).html(result.value);
            Expnder();
        }
        ...
        function FillUnitDRP(id) {
            ...
            var sv = $('#' + ddlCat).val();
            var result = NewFPOrderVendor.FillUnitDRP(id, sv);
            var getDivFEItems = GetClientID("divFPOItems").attr("id");
            $('#' + getDivFEItems).html(result.value);
            Expnder();
        }
        ...
        function Myvalidations() {
            ...
            else if (($('[id$=ddlVendor]').val()).trim() == '00000000-0000-0000-0000-000000000000') {
                ErrorMessage('FPO Floated To is Required.');
                $('[id$=ddlVendor]').focus();
                return false;
            }
            else if ($('#ctl00_ContentPlaceHolder1_Lstfqn :selected').length == 0) {
            ...
            if (rowCount > 0) {
                var result = NewFPOrderVendor.ValidateRowsBeforeSave();
    ...
    <script type="text/javascript">
        function CheckFPOOrderNo() {
            ...
            var result = NewFPOrderVendor.CheckFPOOrderNo(enqNo);
        ...
        function CheckItem(ID) {
            ...
            var ItemID = $('#HItmID' + ID).val();
            var result = NewFPOrderVendor.CheckItem(IsChecked, ID, ItemID);
            $('#tblItems').html('');
