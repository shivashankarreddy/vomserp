<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true"
    CodeBehind="NewFPOrder.aspx.cs" Inherits="VOMS_ERP.Purchases.NewFPOrder" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function uploadComplete() {
            var result = NewFPOrder.AddItemListBox();
        ...
        $('#lnkdelete').click(function () {
            if ($('#lbItems').val() != null) {
                ...
                if (IsDelete) {
                    var result = NewFPOrder.DeleteItemListBox($('#lbItems').val());
    ...
    <script type="text/javascript">
        ...
        function doConfirmPayment(id) {
            if (confirm("Are you sure you want to Delete Payment?")) {
                var result = NewFPOrder.PaymentDeleteItem(id);
        ...
        function getPaymentValues(RNo) {
            ...
            if (PercAmt != '0' && PercAmt != '' && Desc != '') {
                var result = NewFPOrder.PaymentAddItem(RNo, PercAmt, Desc);
                var getdivPaymentTerms = GetClientID("divPaymentTerms").attr("id");
                $('#' + getdivPaymentTerms).html(result.value);
                if ($('[id$=HfMessage]').val() != '') {
    ...
    <script type="text/javascript">
        ...
        function doConfirm(id) {
            if (confirm("Are you sure you want to Delete?")) {
                var result = NewFPOrder.DeleteItem(id);
        ...
        function doConfirmRegret(id) {
            if (confirm("Are you sure you want to Regret?")) {
                ...
                var result = NewFPOrder.RegretItem(id, fenqid);
                if (result.value == true) {

                    $('[id$=divMyMessage]').append('<span class="Success">Item Regretted Successfully.');


                    var result1 = NewFPOrder.BindGridView(0);
                    var getDivFEItems = GetClientID("divFPOItems").attr("id");
        ...
        function CheckQTY(id, isnew) {
            ...
            if (parseFloat(txtQuantity.val()) > parseFloat(txtQTY.val())) {
                if (!confirm("Are you sure you want to enter more than '" + txtQTY.val() + "' Quantity...?")) {
                    txtQuantity.val(txtQTY.val());
                }
            }
            AddItemColumn(id, isnew);
        }

        function Checkrate(id, isnew) {
            var txtRate = $('#txtRate' + id);
            //var txtQTY = $('#hfQTY' + id);
            //if (parseFloat(txt.val()) > parseFloat(txtQTY.val())) {
            //if (!confirm("Are you sure, you want to change Rate...?")) {
                txtRate.val(txtRate.val());
            //}
            //}
            AddItemColumn(id, isnew);
        }
        ...
        function AddItemColumn(id, isnew) {
            ...
            if (sv == undefined && PrtNo == '' && spec == '' && Make == '' && Qty == '' && Rate == 0) {
                ...
                if (Rate == undefined || Rate == '0')
                    Rate = 0;
                //var test = e.tagName; 
                var result = NewFPOrder.AddNewRow(id, sv, PrtNo, spec, Make, Qty, Rate, Units, Rmrks, isnew);
                var getDivFEItems = GetClientID("divFPOItems").attr("id");
                $('#' + getDivFEItems).html(result.value);
            }
            Expnder();
        ...
        function NextPage() {
            ...
            if (ddlRowsChanged == 'NAN' || ddlRowsChanged <= 0) {
                ...
                var getDivFEItems = GetClientID("divFPOItems").attr("id");
                var result = NewFPOrder.NextPage(hfCurrentPage, ddlRowsChanged);
                $('#' + getDivFEItems).html(result.value);
                Expnder();
            }
        };
        ...
        function PrevPage() {
            ...
            if (ddlRowsChanged == 'NAN' || ddlRowsChanged <= 0) {
                ...
                var result = NewFPOrder.PrevPage(hfCurrentPage, ddlRowsChanged);
                var getDivFEItems = GetClientID("divFPOItems").attr("id");
                $('#' + getDivFEItems).html(result.value);
                Expnder();
            }
        ...
        function RowsChanged() {
            ...
            var result = NewFPOrder.RowsChanged(hfCurrentPage, ddlRowsChanged);
            var getDivFEItems = GetClientID("divFPOItems").attr("id");
            $('#' + getDivFEItems).html(result.value);
            Expnder();
        }
        ...
        function DisplayRefFPO() {
            ...
            var result = NewFPOrder.ChkBoxRptdFPOMode($('[id$=chkbIRO]')[0].checked);
        }
        function FillItemDRP(id) {
            var ddlCat = GetClientID("ddl" + (parseInt(id))).attr("id");
            var sv = $('#' + ddlCat).val();
            var result = NewFPOrder.FillItemDRP(id, sv);
            var getDivFEItems = GetClientID("divFPOItems").attr("id");
            $('#' + getDivFEItems).html(result.value);
            Expnder();
        }
        ...
        function FillUnitDRP(id) {
            ...
            var sv = $('#' + ddlCat).val();
            var result = NewFPOrder.FillUnitDRP(id, sv);
            var getDivFEItems = GetClientID("divFPOItems").attr("id");
            $('#' + getDivFEItems).html(result.value);
            Expnder();
        }
        ...
        function Myvalidations() {
            ...
            var Customer = $('[id$=ddlcustomer] :selected').text();
            if (Customer == "DCGN" || Customer == "DCM") {
                if ($("[id$=Chkbivac]").is(':checked') == false) {
                    ErrorMessage('Bivac Is Mandatory for Selected Customer Plant');
                    $('[id$=Chkbivac]').focus();
                    //return false;
                }
            }
            else if (Customer == "DCB" || Customer == "DCC" || Customer == "DCT" || Customer == "FORTIA") {
                if ($("[id$=Chkcotecna]").is(':checked') == false) {
                    ErrorMessage('Cotecna Is Mandatory for Selected Customer Plant');
                    $('[id$=Chkcotecna]').focus();
                    //  return false;
                }
            }
        ...
        function fnOpen(id, rowIndex) {
            ...
            if (ED == '00000000-0000-0000-0000-000000000000') {
                window.open("../Masters/TermsNConditions.aspx?TAr=General&FQQuteID=" + LQuoteID + "&FPO=true", "508", "width=1000,height=500, top=" + y + ", left=" + x + ""); //../Enquiries/AddItems.aspx
            }
            else {
                window.open("../Masters/TermsNConditions.aspx?TAr=General&FQQuteID=" + ED + "&FPO=false", "508", "width=1000,height=500, top=" + y + ", left=" + x + ""); //../Enquiries/AddItems.aspx
            }
    ...
    <script type="text/javascript">
        function CheckFPOOrderNo() {
            ...
            var result = NewFPOrder.CheckFPOOrderNo(enqNo);
        ...
        function CheckItem(ID) {
            ...
            var result = NewFPOrder.CheckItem(IsChecked, ID);
            $('#tblItems').html('');
